...
5. Refinement: Use local Ollama LLM to validate and explain matches
...
def link_segments_to_documents(
    ...
    Args:
        ...
        documents: List of document objects (slides/pages)
        use_llm: Whether to use Ollama LLM for refinement (default: True)
    ...
    if use_llm:
        print("[AlignmentEngine] Checking Ollama readiness...")
        check_ollama_ready()
    ...
    for idx, segment in enumerate(video_segments):
        print(f"[AlignmentEngine] Processing segment {idx + 1}/{len(video_segments)}")
        
        segment_text = segment.get("text", "")
        if not segment_text:
            continue
        
        segment_id = segment.get("segment_id", segment.get("id", idx))
...
def process_video_and_document(
    ...
    Args:
        ...
        use_llm: Whether to use Ollama for refinement
